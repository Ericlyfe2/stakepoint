...
export default function WithdrawPage() {
  ...
  const [showExtraDeposit, setShowExtraDeposit] = useState(false); // Stage 2/3: 10% deposit not yet met
  const [showStageRule, setShowStageRule] = useState(false);       // Stage 2/3 "conditions met" rule modal (blocks below Stage 4)
  ...
  const handleSubmit = async (e) => {
    ...
    if (stage === 2 || stage === 3) {
      ...
      // exists at Stage 4 / VIP). Order of conditions:
      //   1. the 10% extra-deposit rule ("Additional deposit required") — the
      //      modal only shows while a deposit is actually still needed, so its
      //      "Still needed" figure is never GHS 0.00

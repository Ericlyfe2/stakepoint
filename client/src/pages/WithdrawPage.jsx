...
export default function WithdrawPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (stage === 2 || stage === 3) {
      ...
      const depositMet = totalDeposited >= required;
      if (isBlocked && depositMet) {
        setShowBlocked(true);
        return;
      }
  ...
  // "Still needed" is the extra approved deposit required for this withdrawal
  // (10% of the amount) — the existing approved-deposit credit is displayed
  // for context but is not deducted from the requirement, so this figure is
  // always the required amount, never GHS 0.00.
  const extraStillNeeded = extraRequired;

...
export async function sendOtp(to, code, purpose = 'verify') {
  const subjectMap = {
    verify:        'Your Xenbet verification code',
    'reset':       'Reset your Xenbet password',
    'login':       'Xenbet login verification',
  };
  const subject = subjectMap[purpose] || 'Your Xenbet code';
  const text = `Your Xenbet code is ${code}. It expires in 10 minutes.\n\nIf you did not request this, ignore this email.`;
  const html = `<div style="font-family:system-ui,sans-serif">
    <h2 style="color:#ffb800">Xenbet</h2>
